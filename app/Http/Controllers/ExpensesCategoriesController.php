...
class ExpensesCategoriesController extends Controller
{
    ...
    {
        $query = ExpenseCategory::with('expenseList.category','expenseList.subCategory','subCategoryList.expenseList');
        ...
        if ($request->has('sort_column') && $request->has('sort_order')) {
            ...
            if (in_array($sortColumn, ['name', 'remarks'])) {
    ...
    public function manage(Request $request)
    {
        $validatedData = $request->validate([
            'id' => 'nullable|numeric|min:1|exists:expense_categories,id',
    ...
    public function update($validatedData)
    {
        ...
        try{
            ...
            $update = ExpenseCategory::findOrFail($validatedData['id']);
